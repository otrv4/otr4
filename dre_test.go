...
func (s *DualReceiverEncryptionSuite) Test_HashToScalar(c *C) {
	...
	exp := ed448.NewDecafScalar([56]byte{
		0x1e, 0xda, 0x47, 0xce, 0x5a, 0x2a, 0x10,
		0xdb, 0x67, 0x8a, 0x38, 0x2c, 0xe2, 0x70,
		0x2f, 0xea, 0x92, 0x8d, 0x6a, 0x4c, 0x11,
		0x27, 0xfd, 0x7c, 0xb0, 0x6f, 0x1a, 0x0b,
		0x71, 0x82, 0x6b, 0x90, 0xe3, 0x6b, 0xdd,
		0x7d, 0x17, 0xab, 0xfd, 0x9e, 0xad, 0xf2,
		0x04, 0x0d, 0x97, 0x19, 0x46, 0x09, 0x3d,
		0xb3, 0xa3, 0x67, 0xca, 0x01, 0x8d, 0x95,
	})
...
func (s *DualReceiverEncryptionSuite) Test_Concat(c *C) {
	...
	scalar := ed448.NewDecafScalar([56]byte{
	...
	c.Assert(concat(empty, bytes, scalar, point), DeepEquals, exp)
}

func fixedRand() []ed448.Scalar {
	t1 := ed448.NewDecafScalar([56]byte{
		0xea, 0x25, 0xbc, 0x1d, 0x8d, 0x18, 0x2f, 0xe2,
		0x33, 0xe4, 0xd1, 0x8c, 0x58, 0xac, 0x3a, 0x75,
		0x32, 0x7a, 0xb0, 0x91, 0xcf, 0x85, 0x81, 0xf8,
		0x2c, 0xc5, 0xf3, 0x55, 0x3d, 0x32, 0x2d, 0x3e,
	...
	c2 := ed448.NewDecafScalar([56]byte{
	...
	c3 := ed448.NewDecafScalar([56]byte{
	...
	r2 := ed448.NewDecafScalar([56]byte{
	...
	r3 := ed448.NewDecafScalar([56]byte{
	...
	return []ed448.Scalar{t1, c2, c3, r2, r3}
}
...
func (s *DualReceiverEncryptionSuite) Test_Auth(c *C) {
	...
	sigma := auth(fixedRand, p1, p2, p3, secA, message)

	c.Assert(sigma, DeepEquals, expSigma)
}
